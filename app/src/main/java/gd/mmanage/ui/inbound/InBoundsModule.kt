...
class InBoundsModule : BaseModule {
     ...
     * @param model入库信息集合
     * */
    fun add_prat(model: InBoundModel) {
        HttpUtils<PartsModel>().post(url.get_storage + "AddStorage", command.parts, UtilControl.change(model), this)
    }
    ...
    fun get_in_bounds(map: HashMap<String, String>) {
        HttpUtils<List<InBoundModel>>().post(url.get_storage + "SearchStorage", command.parts + 1, map, this)
    }
}